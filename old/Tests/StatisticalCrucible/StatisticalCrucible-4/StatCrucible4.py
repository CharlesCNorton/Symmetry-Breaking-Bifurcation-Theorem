...
def advanced_statistical_analysis(num_simulations=100000, noise_levels=[0.0, 0.1, 0.2, 0.3, 1.0], num_folds=5):
    ...
    for noise in noise_levels:
        ...
        for obj in objects:
            ...
            # Logarithmic and exponential model fitting
            try:
                params_log, _ = curve_fit(logarithmic_model, X_train, y_train)
                log_pred = logarithmic_model(X_test, *params_log)
                ...
                print(f"{Fore.RED}{obj['name']} Logarithmic Model failed: {e}{Style.RESET_ALL}")

            try:
                params_exp, _ = curve_fit(exponential_model, X_train, y_train)
                exp_pred = exponential_model(X_test, *params_exp)
                exp_mse = mean_squared_error(y_test, exp_pred)
                exp_r2 = r2_score(y_test, exp_pred)
                print(f"{Fore.MAGENTA}{obj['name']} Exponential Model MSE: {exp_mse:.5f}, R-squared: {exp_r2:.5f}{Style.RESET_ALL}")
            except RuntimeError as e:
                print(f"{Fore.RED}{obj['name']} Exponential Model failed: {e}{Style.RESET_ALL}")
